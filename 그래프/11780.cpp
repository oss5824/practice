...
int main()
{
	...
	for (int i = 1; i <= n; i++)
		for (int j = 1; j <= n; j++)
			for (int k = 1; k <= n; k++)
			{
				...
				int y = M[j][i].second;
				string w = M[i][k].first.second; int len = w.length();
				int index = w.find(' ', 0);
				string p = w.substr(index + 1, len - index);
				string s = M[j][i].first.second + p;
	...
	for (int i = 1; i <= n; i++)
		for (int j = 1; j <= n; j++)
		{
			...
			{
				string s = M[i][j].first.second; int pos = 0, cnt = 0;
				while ((pos = s.find(' ', pos)) != string::npos) { cnt++; pos++; }
				cout << cnt << " " << M[i][j].first.second << "\n";
			}
		}
	return 0;
}