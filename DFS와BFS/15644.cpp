...
queue<pair<pp, pair<int, string>>>q;
...
void Setting(int x1, int y1, int x2, int y2, int num,string s)
{
	pp next = { {x1,y1},{x2,y2} };
	visit[y1][x1][y2][x2] = true; q.push({ next,{num + 1,s} });
}
void bfs()
{
	q.push({ { red,blue },{0,""} });
	...
	while (!q.empty()) {
		...
		string trace = q.front().second.second;
		if (Map[redY][redX] == 'O') { cout << num << "\n" << trace; return; }
		q.pop();
		...
		for (int i = 0; i < 4; i++) {
			...
			Setting(redX2, redY2, blueX2, blueY2, num, trace + direction[i]);
		}
	}
	cout << "-1";
}
